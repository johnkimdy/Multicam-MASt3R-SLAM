...
class Window(WindowEvents):
    ...
    def __init__(self, states, keyframes, main2viz, viz2main, **kwargs):
        ...
        self.colors = {
            0: [1, 0, 0, 1],  # Red for camera 0 (reference camera)
            1: [1, 1, 0, 1],  # Yellow for camera 1, 
            2: [0, 1, 1, 1],  # Cyan for camera 2
            3: [1, 0, 1, 1],  # Magenta for camera 3
            4: [0, 0, 1, 1],   # Blue for camera 4
            5: [0.5, 0.5, 0.5, 1],  # Gray for camera 5
        }
    ...
    def render(self, t: float, frametime: float):
        ...
        # Add frustums for all cameras
        for cam_id in range(num_cameras):
            # Skip the reference camera as it's already handled
            if cam_id == self.reference_camera_ID:
                continue
                
            cam_frame = self.states.get_frame(cam_id)
            if cam_frame is not None:
                cam_T_WC = as_SE3(cam_frame.T_WC).cpu()
                self.frustums.add(
                    cam_T_WC,
                    scale=self.frustum_scale,
                    color=[0, 1, 0, 1],
        ...
        for kf_idx in range(N_keyframes):
            ...
            # Print camera ID for debugging
            # print(f"(visualizer) Current camera ID: {camid}")
            # Default to red if camera ID not in colors dictionary
            # Use dictionary access instead of get() to ensure we're accessing the correct camera ID
            # If the camera ID is not in the dictionary, default to blue
            color = self.colors[int(camid)]# if camid in colors else [0, 0, 1, 1]
    ...
    def render_ui(self):
        ...
        image_with_text(self.reference_curr_img, size, "Camera 1", same_line=False, border_color = self.colors[0])
        ...
        for cam_id, img_obj in self.curr_imgs.items():
            ...
            try:
                image_with_text(img_obj, size, f"Camera {cam_id + 1}", same_line=False, border_color = self.colors[int(cam_id)])
            except Exception as e:
                print(f"[DEBUG] Error drawing image for camera {cam_id + 1}: {e}")
        imgui.end()
    ...
    def render_pointmap(self, T_WC, w, h, ptex, ctex, itex, use_img=True, depth_bias=0):
        # print("[DEBUG] render_pointmap called with:")
        # print(f"  w: {w}, h: {h}")
        # print(f"  use_img: {use_img}, depth_bias: {depth_bias}")
        # print(f"  T_WC shape: {T_WC.matrix().shape}")
        ...
        # print(f"  model matrix shape: {model.shape}")
        # Print the last column (translation) of the 4x4x1 transformation matrix
        # print(f"  Last column (translation): {model[3, :, 0]}")
        ...
        if "depth_bias" in self.pointmap_prog:
            ...
            # print(f"  depth_bias: {depth_bias}")

        # print(f"[DEBUG] Rendering {w * h} vertices")
